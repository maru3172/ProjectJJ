...
bool DBConnection::Connect(SQLHENV henv, const WCHAR* connectionString)
{
	...
	SQLRETURN ret = ::SQLDriverConnectW(_connection, NULL, reinterpret_cast<SQLWCHAR*>(stringBuffer), _countof(stringBuffer), OUT reinterpret_cast<SQLWCHAR*>(resultString), _countof(resultString), OUT &resultStringLen, SQL_DRIVER_NOPROMPT);
...
{
	return BindParam(paramIndex, SQL_C_DOUBLE, SQL_C_DOUBLE, 0, value, index);
}

bool DBConnection::BindParam(int32 paramIndex, int8* value, SQLLEN* index)
{
	return BindParam(paramIndex, SQL_C_TINYINT, SQL_C_TINYINT, size32(int8), value, index);
}
...
void DBConnection::HandleError(SQLRETURN ret)
{
	...
	while (true)
	{
		errorRet = ::SQLGetDiagRecW(SQL_HANDLE_STMT, _statement, index, sqlState, OUT &nativeError, errMsg, _countof(errMsg), OUT &msgLen);
